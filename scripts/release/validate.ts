...
if (configErrors.length === 0 && changed.length === 0) {
  console.log(
    `Release metadata is in sync. js-compound-engineering currently has ${counts.agents} agents, ${counts.skills} skills, and ${counts.mcpServers} MCP server${counts.mcpServers === 1 ? "" : "s"}.`,
  )
  process.exit(0)
}
...
if (changed.length > 0) {
  ...
  console.error(
    `Current js-compound-engineering counts: ${counts.agents} agents, ${counts.skills} skills, ${counts.mcpServers} MCP server${counts.mcpServers === 1 ? "" : "s"}.`,
  )
}
process.exit(1)
