...
Continue with /js-ce:plan when ready.
...
`,
    ...
    await fs.writeFile(
      ...
      "Reference docs still mention /js-ce:plan here.\n",
    )

    const bundle: CodexBundle = {
      prompts: [],
      skillDirs: [{ name: "js-ce:brainstorm", sourceDir: sourceSkillDir }],
      generatedSkills: [],
      invocationTargets: {
        promptTargets: {
          "js-ce-plan": "js-ce-plan",
          "workflows-plan": "js-ce-plan",
    ...
    const installedSkill = await fs.readFile(
      path.join(tempRoot, ".codex", "skills", "js-ce-brainstorm", "SKILL.md"),
      "utf8",
    )
    expect(installedSkill).toContain("/prompts:js-ce-plan")
    ...
    const notes = await fs.readFile(
      path.join(tempRoot, ".codex", "skills", "js-ce-brainstorm", "notes.md"),
      "utf8",
    )
    expect(notes).toContain("/js-ce:plan")
  })
...
`,
    ...
    const installedSkill = await fs.readFile(
      path.join(tempRoot, ".codex", "skills", "js-ce-plan", "SKILL.md"),
      "utf8",
    )
