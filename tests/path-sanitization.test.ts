...
const pluginRoot = path.join(process.cwd(), "plugins", "js-compound-engineering")

describe("sanitizePathName", () => {
  test("replaces colons with hyphens", () => {
    expect(sanitizePathName("js-ce:brainstorm")).toBe("js-ce-brainstorm")
    expect(sanitizePathName("js-ce:plan")).toBe("js-ce-plan")
  })
