...
async function createTestRepo(): Promise<string> {
  ...
  const pluginRoot = path.join(repoRoot, "plugins", "js-compound-engineering")
...
describe("plugin-path", () => {
  test("clones a branch to a stable cache path", async () => {
    ...
    const cacheDir = path.join(tempHome, ".cache", "js-compound-engineering", "branches", "js-compound-engineering-feat~test-branch")
    const pluginDir = path.join(cacheDir, "plugins", "js-compound-engineering")
  ...
  test("sanitizes branch names with slashes into stable directory names", async () => {
    ...
    expect(stdout).toContain("js-compound-engineering-feat~deep~nested~branch")
    expect(stderr).toContain("claude --plugin-dir")
  })
  ...
  test("updates existing checkout on re-run", async () => {
    ...
    const markerPath = path.join(repoRoot, "plugins", "js-compound-engineering", "MARKER.txt")
    ...
    const cacheDir = path.join(tempHome, ".cache", "js-compound-engineering", "branches", "js-compound-engineering-feat~update-test")
    ...
    const cachedMarker = path.join(cacheDir, "plugins", "js-compound-engineering", "MARKER.txt")
