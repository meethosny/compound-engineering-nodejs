...
async function makeFixtureRoot(): Promise<string> {
  ...
  await mkdir(path.join(root, "plugins", "js-compound-engineering", "agents", "review"), {
    recursive: true,
  })
  await mkdir(path.join(root, "plugins", "js-compound-engineering", "skills", "js-ce-plan"), {
    recursive: true,
  })
  await mkdir(path.join(root, "plugins", "js-compound-engineering", ".claude-plugin"), {
    recursive: true,
  })
  await mkdir(path.join(root, "plugins", "js-compound-engineering", ".cursor-plugin"), {
    recursive: true,
  })
  await mkdir(path.join(root, "plugins", "js-coding-tutor", ".claude-plugin"), {
    recursive: true,
  })
  await mkdir(path.join(root, "plugins", "js-coding-tutor", ".cursor-plugin"), {
    recursive: true,
  })
  ...
  await writeFile(
    path.join(root, "plugins", "js-compound-engineering", "agents", "review", "agent.md"),
    "# Review Agent\n",
  )
  await writeFile(
    path.join(root, "plugins", "js-compound-engineering", "skills", "js-ce-plan", "SKILL.md"),
    "# ce:plan\n",
  )
  await writeFile(
    path.join(root, "plugins", "js-compound-engineering", ".mcp.json"),
    JSON.stringify({ mcpServers: { context7: { command: "ctx7" } } }, null, 2),
  )
  await writeFile(
    path.join(root, "plugins", "js-compound-engineering", ".claude-plugin", "plugin.json"),
    JSON.stringify({ version: "2.42.0", description: "old" }, null, 2),
  )
  await writeFile(
    path.join(root, "plugins", "js-compound-engineering", ".cursor-plugin", "plugin.json"),
    JSON.stringify({ version: "2.33.0", description: "old" }, null, 2),
  )
  await writeFile(
    path.join(root, "plugins", "js-coding-tutor", ".claude-plugin", "plugin.json"),
    JSON.stringify({ version: "1.2.1" }, null, 2),
  )
  await writeFile(
    path.join(root, "plugins", "js-coding-tutor", ".cursor-plugin", "plugin.json"),
    JSON.stringify({ version: "1.2.1" }, null, 2),
  )
  await writeFile(
    ...
    JSON.stringify(
      {
        ...
        plugins: [
          { name: "js-compound-engineering", version: "2.41.0", description: "old" },
          { name: "js-coding-tutor", version: "1.2.0", description: "old" },
        ],
      },
      null,
      2,
...
describe("release metadata", () => {
  ...
  test("detects cross-surface version drift even without explicit override versions", async () => {
    ...
    expect(changedPaths).toContain(path.join(root, "plugins", "js-compound-engineering", ".cursor-plugin", "plugin.json"))
