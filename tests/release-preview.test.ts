...
describe("release preview", () => {
  test("uses changed files to determine affected components and next versions", async () => {
    ...
    const preview = await buildReleasePreview({
      ...
      files: ["plugins/js-compound-engineering/skills/js-ce-plan-beta/SKILL.md"],
    })

    expect(preview.components).toHaveLength(1)
    expect(preview.components[0].component).toBe("js-compound-engineering")
    expect(preview.components[0].inferredBump).toBe("patch")
    expect(preview.components[0].nextVersion).toBe(bumpVersion(versions["js-compound-engineering"], "patch"))
  })
  ...
  test("supports per-component overrides without affecting unrelated components", async () => {
    ...
    const preview = await buildReleasePreview({
      ...
      files: ["plugins/js-coding-tutor/README.md"],
      overrides: {
        "js-coding-tutor": "minor",
      },
    })

    expect(preview.components).toHaveLength(1)
    expect(preview.components[0].component).toBe("js-coding-tutor")
    expect(preview.components[0].inferredBump).toBe("patch")
    expect(preview.components[0].effectiveBump).toBe("minor")
    expect(preview.components[0].nextVersion).toBe(bumpVersion(versions["js-coding-tutor"], "minor"))
  })
