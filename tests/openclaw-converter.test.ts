...
const fixturePlugin: ClaudePlugin = {
  root: "/tmp/plugin",
  manifest: { name: "js-compound-engineering", version: "1.0.0", description: "A plugin" },
  agents: [
    {
...
describe("convertClaudeToOpenClaw", () => {
  ...
  test("manifest includes plugin id, display name, and skills list", () => {
    ...
    expect(bundle.manifest.id).toBe("js-compound-engineering")
    expect(bundle.manifest.name).toBe("Js Compound Engineering")
  ...
  test("package.json uses plugin name and version", () => {
    ...
    expect(bundle.packageJson.name).toBe("openclaw-js-compound-engineering")
  ...
  test("manifest skill paths use sanitized names matching filesystem output", () => {
    ...
    expect(bundle.manifest.skills).toContain("skills/js-ce-plan")
    expect(bundle.manifest.skills).toContain("skills/cmd-workflows-plan")
  })
})
