...
if [ "$#" -ge 2 ] && [ "$1" = "pr" ] && [ "$2" = "view" ]; then
  printf '%s' '{"baseRefName":"main","url":"https://github.com/meethosny/compound-engineering-nodejs/pull/123"}'
  exit 0
fi
exit 1
`,
...
describe("resolve-base.sh", () => {
  test("prefers the PR base remote from gh metadata over origin", async () => {
    ...
    await runGit(
      ["remote", "add", "upstream", "git@github.com:meethosny/compound-engineering-nodejs.git"],
      repoRoot,
    )
  ...
  test("unshallows the PR base remote in a detached shallow checkout", async () => {
    ...
    const upstreamBare = path.join(
      ...
      "github.com",
      "meethosny",
      "compound-engineering-nodejs.git",
    )
