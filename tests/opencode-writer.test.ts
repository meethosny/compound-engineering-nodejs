...
describe("writeOpenCodeBundle", () => {
  ...
  test("rewrites FQ agent names in copied skill markdown (#477)", async () => {
    ...
    await fs.writeFile(
      ...
      "---\nname: test-skill\n---\n\n- `js-compound-engineering:review:coherence-reviewer`\n"
    )
    await fs.writeFile(
      path.join(refsDir, "agents.md"),
      "Use `js-compound-engineering:research:repo-research-analyst` for codebase analysis.\n"
    )
    ...
    expect(skillContent).not.toContain("js-compound-engineering:review:coherence-reviewer")
    ...
    expect(refContent).not.toContain("js-compound-engineering:research:repo-research-analyst")
  })
