...
>
> All your tutorials live in one central library (`~/coding-tutor-tutorials/`) that works across all your projects. Use `/js-teach-me` to learn something new, `/js-quiz-me` to test your retention with spaced repetition.
...
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/skills/js-coding-tutor/scripts/setup_tutorials.py
```
...
- **Survey existing knowledge**: Run `python3 ${CLAUDE_PLUGIN_ROOT}/skills/js-coding-tutor/scripts/index_tutorials.py` to understand what concepts have been covered, at what depth, and how well they landed (understanding scores). Optionally, dive into particular tutorials in `~/coding-tutor-tutorials/` to read them.
...
```bash
python3 ${CLAUDE_PLUGIN_ROOT}/skills/js-coding-tutor/scripts/create_tutorial.py "Topic Name" --concepts "Concept1,Concept2"
```
...
- Open: "Quiz me on something" → run `python3 ${CLAUDE_PLUGIN_ROOT}/skills/js-coding-tutor/scripts/quiz_priority.py` to get a prioritized list based on spaced repetition, then choose what to quiz
