...
**Skills (new):**
- `modern-nodejs-style` - Write Node.js code with modern, pragmatic patterns (MVP-focused, minimal dependencies, async-first)
- `sindre-sorhus-package-writer` - Write npm packages following Sindre Sorhus's proven patterns (ESM-first, zero deps, TypeScript definitions)
- `dspy-python` - Build type-safe LLM applications with Stanford's original Python DSPy framework
...
**Commands updated:**
- `lint` agent - Now uses ESLint/Prettier instead of StandardRB/ERBLint
- `generate_command` - Changed `bin/rails test` → `npm test`, `bundle exec standardrb` → `npx eslint .`
- `playwright-test` - Changed Rails server commands to Node.js (`npm run dev`)
- `plan_review` - Updated to use `modern-nodejs-reviewer` and `kieran-nodejs-reviewer`
- `workflows:review` - Updated parallel agents list for Node.js

**Documentation updated:**
- README.md - All agent and skill descriptions updated for Node.js
- Framework references changed from Rails to Express/Fastify
...
- **`/js-workflows:review` command** - Section 7 now detects project type (Web, iOS, or Hybrid) and offers appropriate testing. Web projects get `/playwright-test`, iOS projects get `/xcode-test`, hybrid projects can run both.
...
- **`/js-workflows:review` command** - Added optional Playwright testing phase (Section 7). After review agents complete, offers to spawn `/playwright-test` as a subagent to verify affected pages in a real browser.
...
- **`/js-workflows:review` command** - Added conditional agents section. Now automatically runs `data-migration-expert` and `deployment-verification-agent` when PR contains database migrations (`db/migrate/*.rb`), data backfills, or ID/enum mapping changes.
...
- **Command naming convention** - Workflow commands now use `workflows:` prefix to avoid collisions with built-in Claude Code commands:
  - `/js-workflows:plan` (was `/plan`)
  - `/js-workflows:review` (was `/review`)
  - `/js-workflows:work` (was `/work`)
  - `/js-workflows:compound` (was `/compound`)
