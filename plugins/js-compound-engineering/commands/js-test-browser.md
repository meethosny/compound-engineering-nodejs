...
**DO NOT use Playwright MCP tools (mcp__playwright__*).**

This command uses the `agent-browser` CLI exclusively. The agent-browser CLI is a Bash-based tool from Vercel that runs headless Chromium. It is NOT the same as Playwright MCP.

If you find yourself calling `mcp__playwright__*` tools, STOP. Use `agent-browser` Bash commands instead.
...
<requirements>
- Local development server running (e.g., `npm run dev`, `npm start`, `node server.js`)
...
Map changed files to testable routes based on the Node.js framework detected:
...
| `src/pages/users/[id].tsx` | `/users/:id` (test with sample ID) |
| `src/pages/api/*` | API routes (skip browser testing) |
| `src/components/*` | Find pages importing these components |
...
| `src/app/users/[id]/page.tsx` | `/users/:id` (test with sample ID) |
| `src/app/layout.tsx` | All pages (test homepage) |
| `src/components/*` | Find pages importing these components |

**Express/Fastify/Hono:**
| File Pattern | Route(s) |
|-------------|----------|
| `routes/*.ts` or `routes/*.js` | Parse routes defined in file |
| `src/routes/*.ts` | Parse routes defined in file |
| `views/*.ejs` or `views/*.pug` | Routes rendering these templates |
| `public/*` | Static assets (skip browser testing) |
...
| File Pattern | Route(s) |
|-------------|----------|
| `src/styles/*`, `styles/*` | Visual regression on key pages |
| `src/components/*`, `components/*` | Pages using those components |
...
- Next.js: `npm run dev`
- Express/Fastify/Hono: `npm start` or `node server.js`
- Remix: `npm run dev`
...
For each affected route, use agent-browser CLI commands (NOT Playwright MCP):
...
**ALWAYS use these Bash commands. NEVER use mcp__playwright__* tools.**
