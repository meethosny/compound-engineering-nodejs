...
**Headless mode** changes the interaction model, not the classification boundaries. js-document-review still applies the same judgment about which tier each finding belongs in. The only difference is how non-safe_auto findings are delivered:
...
```
Skill("js-document-review", "mode:headless docs/plans/my-plan.md")
```
...
**If no document is specified (headless mode):** Output "Review failed: headless mode requires a document path. Re-invoke with: Skill(\"js-document-review\", \"mode:headless <path>\")" without dispatching agents.
...
Cross-session persistence is out of scope. A new invocation of js-document-review on the same document starts with a fresh round 1 and no carried primer, even if prior sessions deferred findings into the document's Open Questions section.
