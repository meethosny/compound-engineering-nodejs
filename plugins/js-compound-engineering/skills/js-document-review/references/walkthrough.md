...
Cross-session persistence is out of scope. Mirrors `js-ce-code-review`'s walk-through state rules.
...
The walk-through is operationally read-only with respect to the project except for three permitted writes: the in-memory Apply set / decision list (managed by the orchestrator), the in-doc Open Questions appends (external side effects managed by `references/open-questions-defer.md`), and the end-of-walk-through batch document edits (the orchestrator's final Apply pass). Persona agents remain strictly read-only. Unlike `js-ce-code-review`, there is no fixer subagent — the orchestrator owns the document edit directly.
