...
- `{ "validated": true, "reason": "Cited line is new in this diff and lacks the ownership guard used by parallel route handlers." }`
- `{ "validated": false, "reason": "Line 87 already guards user.email with a truthiness check; the undefined access the finding describes cannot occur." }`
- `{ "validated": false, "reason": "Cited line dates to 2024-08 (pre-existing); diff does not modify or interact with it." }`
- `{ "validated": false, "reason": "The HTTP client retries on timeout via its default config; no application-level retry needed." }`
