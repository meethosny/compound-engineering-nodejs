...
If `agent-browser` is not installed, inform the user: "`agent-browser` is not installed. Run `/js-ce-setup` to install required dependencies." Then fall back to a lower tier (static screenshots or skip).
...
- Check `package.json` for Node.js (`npm run dev`) or Sinatra
