...
If `agent-browser` is not installed, inform the user: "`agent-browser` is not installed. Run `/js-ce-setup` to install required dependencies." Then skip to the CLI or fallback sections below.
