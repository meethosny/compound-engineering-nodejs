...
| 3. Environment guard | Refuse dangerous operations in contexts where they make no sense | The operation can be catastrophic if run in the wrong environment | In tests (`process.env.NODE_ENV === 'test'`), refuse `git init` outside the OS temp dir |
