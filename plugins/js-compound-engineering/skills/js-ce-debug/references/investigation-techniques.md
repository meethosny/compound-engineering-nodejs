...
### Express / Fastify
- Check middleware and hooks: Express `app.use` ordering and Fastify `onRequest`/`preHandler` hooks execute implicitly and can alter the request/response
- Inspect the middleware stack: log `app._router.stack` (Express) or run `fastify.printRoutes()` / `printPlugins()` (Fastify) to see the full chain
- Check ORM query generation: enable Prisma query logging (`log: ['query']`) or call `.toSQL()` on a Knex query builder
- Use a structured logger (pino/winston) with request-scoped child loggers for request tracing
...
For test runs, most test runners integrate with the above — e.g., `node --inspect-brk $(which jest)`, `pytest --pdb`, `vitest --inspect-brk`, `dlv test`. Prefer the runner's integration over trying to attach post-hoc.
