# `/js-ce-optimize` Usage Guide

## What This Skill Is For

`/js-ce-optimize` is for hard engineering problems where:
...
Use `/js-ce-optimize` when the problem looks like:
...
`/js-ce-optimize` is usually the wrong tool when:
...
```text
Use /js-ce-optimize to find the smallest memory setting that keeps this service stable under our load test.
...
```text
Use /js-ce-optimize to improve issue and PR clustering quality.
...
```text
Use /js-ce-optimize to create a summarization prompt for issues and PRs that minimizes token spend while still producing summaries that are good enough for downstream clustering.
