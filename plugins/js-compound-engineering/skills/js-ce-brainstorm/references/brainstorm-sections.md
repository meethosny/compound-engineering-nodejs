...
artifact produced by `js-ce-brainstorm`.
...
`js-ce-plan` later enriches. It enables three audiences to act:

- **The planning agent** (`js-ce-plan` or a human) produces an implementation
...
New `js-ce-brainstorm` outputs live under `docs/plans/` and use the unified plan
artifact contract:

- **Path:** `docs/plans/YYYY-MM-DD-NNN-<type>-<topic>-plan.<md|html>`.
- **`artifact_contract: js-ce-unified-plan/v1`**.
- **`artifact_readiness: requirements-only`**.
- **`product_contract_source: js-ce-brainstorm`**.
...
look executable and waste downstream tokens. `js-ce-plan` adds those sections when
...
- Any durable decisions made during the dialogue can flow naturally to
  downstream artifacts (`js-ce-plan`, the commit message, `docs/solutions/`)
...
decision flows to `js-ce-plan` (or directly to commit message, or to
...
- **Success Criteria** — include when there are quality / metric / handoff
  ...
  one voice"), process / handoff quality ("js-ce-doc-review can act on this
...
- **`title`** — the artifact's descriptive name with a ` - Plan` suffix
  ...
  stays stable when `js-ce-plan` enriches it. Do not put a conventional-commit
...
- **`topic`** — kebab-case slug identifying the brainstorm subject (e.g.,
  ...
  as the resume-detection key when `js-ce-brainstorm` scans for an existing
  artifact to continue.
- **`artifact_contract`** — always `js-ce-unified-plan/v1` for new outputs.
- **`artifact_readiness`** — always `requirements-only` for new
  `js-ce-brainstorm` outputs. Do not use `active`, `in_progress`, `completed`,
  or `done`.
- **`product_contract_source`** — always `js-ce-brainstorm`.
...
- **No process exhaust.** No "captured at Phase X" notes, no `## Next Steps`
  pointing to js-ce-plan, no italic provenance lines. Engineering process
