...
It is paired with a section contract (`brainstorm-sections.md`, etc.) that
describes *what* the artifact contains. This reference describes *how* HTML
specifically presents it. The same content rendered by different skills
shares the same HTML principles.
...
consumers that read HTML today (`js-ce:work`, human readers) do so directly;
the agent-consumability rules below make that work. `js-ce-document-review` is
*not* currently an HTML consumer — its mutation mechanics are markdown-only,
so the js-ce:plan handoff gates the doc-review pass to `OUTPUT_FORMAT=md`
...
- **Editable status renders as `<span class="status">{value}</span>`.**
  Downstream tooling (`js-ce:work` shipping flip, future HTML-aware
...
- **Source / composition signal.** A visible footer at the bottom of
  ...
  `<footer class="composition-signal">Composed 2026-05-17T14:23Z by js-ce:plan from <code>docs/brainstorms/...-requirements.md</code></footer>`.
...
- **Requirements** — `<table>` is the default at 5+ uniform items;
  ...
  the same doc.
- **Key Decisions** — repeating cards with the decision ID (when IDed),
  bold decision title (often with inline code for technical identifiers),
  and prose rationale. Flat cards (not collapsibles) — these are reference
  material readers scan, not drill into.
- **Key Flows** — bullets or cards with bold leader labels
  (`**Trigger:**`, `**Actors:**`, `**Steps:**`, `**Covered by:**`).
- **Acceptance Examples** — cards or table rows with the AE-ID as visible
  text and the Given/When/Then or Covers structure spelled out in prose.
...
### Brainstorm diagrams are not directional sketches

Do not add hedging captions or section preambles to brainstorm SVG
diagrams that depict resolved content — phrases like "directional guidance
for review, not specification" do not belong on conceptual diagrams that
render the same authoritative content as the surrounding prose; the
prose-is-authoritative rule already governs disagreement. Hedging language
is reserved for the wireframe affordance below, which carries a *required*
directional caption because the wireframe is explicitly NOT a spec.
...
- **Sticky TOC sidebar with active-section indicator** — available when
  ...
  on repeating cards already cuts the visible scroll length enough that
  a static TOC works — reach for sticky only when collapsibles alone
  don't solve the navigation problem.
- **Within-section sub-nav** for sections containing 6+ repeating cards
  (Key Decisions, Acceptance Examples at large counts). A short list of
  card-anchor links (`<ul>` of `<a href="#r1">R1. ...</a>`) rendered at
...
- **Tinted callout cards** for content that is "different in kind"
  (Deferred, Open Questions, advisory notes) — color-coded left borders
  communicate kind at a glance.

## Agent-consumability rules

Downstream agents that read HTML today (`js-ce:work`, future consumers) read
the HTML file as text linearly, not via DOM extraction. `js-ce-document-review`
is not a current HTML consumer (see opening note). Compose so semantic
understanding is reachable in source:

- **Use semantic HTML over `<div>` soup.** `<article>` per card, `<dl>`
  for metadata pairs, `<table>` for tabular content, `<details>` /
  `<summary>` for collapsibles, `<section>` for top-level doc sections.
  Structure markers carry meaning to a text-reading agent.
- **Render field labels as visible text, not as attributes.** Emit
  `<dt>TRIGGER</dt><dd>...</dd>`, not `<dd data-field="trigger">...</dd>`.
  The label is the semantic anchor.
- **Keep R-IDs, A-IDs, and similar as visible text** in headings and
  table cells, not only as `id=""` attributes. The agent finds "R1." in
  source the same way it finds "R1." in markdown.
- **Match section heading vocabulary to what the section contract
  defines.** When the section contract says "Requirements," the HTML
  heading is "Requirements" — not "What it must do," even if the
  narrative version reads better. Section heading vocabulary is the
  contract downstream consumers grep for. (Editorial re-titles can appear
  as eyebrow labels, sub-headings, or visual framing — but the
  load-bearing section heading matches the contract name.)
...
- **Repeating cards with 3+ instances put secondary content inside
  default-closed `<details>`.** Fully-expanded cards in a long section is
  a failure mode — the reader can't see the card list at a glance. Verify
  by skimming the rendered cards: each `<article>` should render as its
  ID + title + metadata strip with collapsibles below, not as one long
  block.
...
- **Each heading level** is visually distinct from others and from
  inline bold.
- **No template placeholders** (`{skill}`, `<value>`, `[topic title]`)
  leaked into output.
