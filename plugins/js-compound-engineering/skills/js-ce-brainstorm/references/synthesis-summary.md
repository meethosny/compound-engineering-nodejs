...
- **Cheap-now-expensive-later correction** — a scope bet that's cheap to fix now but expensive after the requirements doc lands and js-ce:plan consumes it
...
- "Persist mute state in the existing data store" — implementation detail (js-ce:plan's job), failed granularity rules
...
When the soft-cut fires, use the platform's blocking question tool (`AskUserQuestion` in Claude Code, `request_user_input` in Codex, `ask_user` in Gemini) with two options:
