...
Brainstorming helps answer **WHAT** to build through collaborative dialogue. It precedes `/js-ce-plan`, which enriches the same unified plan artifact with **HOW** to build it.
...
5. **Pipeline override.** When invoked from LFG or any `disable-model-invocation` context, force `OUTPUT_FORMAT=md` regardless of steps 1-4. Downstream consumers (`js-ce-plan`, `js-ce-work`) parse markdown reliably; HTML in pipeline runs is unnecessary friction.
...
The `output:` preference does NOT auto-propagate to `js-ce-plan` on handoff — js-ce-plan re-resolves its own `plan_output` config independently. Because both skills now operate on the same unified artifact, an explicit conversion by `js-ce-plan` must report the old path and new canonical path; pipeline mode may force markdown by writing the canonical markdown plan path and leaving any HTML sibling untouched as non-canonical for automated discovery.

#### 0.1 Resume Existing Work When Appropriate

If the user references an existing brainstorm topic or document, or there is an obvious recent matching unified plan in `docs/plans/` with `artifact_contract: js-ce-unified-plan/v1`, `artifact_readiness: requirements-only`, and `product_contract_source: js-ce-brainstorm`:
- Read the document
...
Historical `docs/brainstorms/*-requirements.{md,html}` files remain legacy inputs for `js-ce-plan`, but new `js-ce-brainstorm` outputs do not write there.
...
When the shape matches — at intake, or whenever later dialogue (Phases 1.3–2) clarifies a request into it — read `references/verdict-routing.md` and follow it: offer the `/js-ce-pov` handoff interactively (never silently switch), invoke `js-ce-pov` on accept, drop the offer and continue the normal workflow unchanged on decline. The reference owns the offer construction, field mapping, and what to pass to `js-ce-pov`.
...
Write to `docs/plans/YYYY-MM-DD-NNN-<type>-<topic>-plan.<md|html>` — extension follows `OUTPUT_FORMAT`. Include `artifact_contract: js-ce-unified-plan/v1`, `artifact_readiness: requirements-only`, and `product_contract_source: js-ce-brainstorm`. Title is `<Name> - Plan` (matching the H1; no conventional-commit prefix). Keep the doc light and standalone-readable: a Goal Capsule (objective, product authority, open blockers) and the Product Contract. Do **not** emit a Goal Launch Block or Reader Index. See `references/brainstorm-sections.md` — which owns the artifact content rules, including repo-relative file paths inside the doc. When confirming in chat, report the written artifact with its absolute path so the reference is clickable.
...
Read `references/handoff.md` now — before presenting any options. The option set and its visibility conditions, the rendering-mode rule, the per-selection dispatch instructions (including what gets passed to `js-ce-plan`), and the closing summary formats all live there — none of them appear in this main body. An improvised menu silently breaks pipeline routing: options surface in states where they must be hidden, and downstream skills receive the wrong payload.
