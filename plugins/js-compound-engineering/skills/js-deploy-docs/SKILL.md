...
```bash
# Count components
echo "Agents: $(ls plugins/js-compound-engineering/agents/*.md | wc -l)"
echo "Skills: $(ls -d plugins/js-compound-engineering/skills/*/ 2>/dev/null | wc -l)"

# Validate JSON
cat .claude-plugin/marketplace.json | jq . > /dev/null && echo "✓ marketplace.json valid"
cat plugins/js-compound-engineering/.claude-plugin/plugin.json | jq . > /dev/null && echo "✓ plugin.json valid"

# Check all HTML files exist
for page in index agents commands skills mcp-servers changelog getting-started; do
  if [ -f "plugins/js-compound-engineering/docs/pages/${page}.html" ] || [ -f "plugins/js-compound-engineering/docs/${page}.html" ]; then
...
```bash
git status --porcelain plugins/js-compound-engineering/docs/
```
...
on:
  push:
    branches: [main]
    paths:
      - 'plugins/js-compound-engineering/docs/**'
  workflow_dispatch:
  ...
  deploy:
    ...
    steps:
      ...
      - uses: actions/upload-pages-artifact@v3
        with:
          path: 'plugins/js-compound-engineering/docs'
...
- [ ] Check deployment at https://meethosny.github.io/compound-engineering-nodejs/
```
