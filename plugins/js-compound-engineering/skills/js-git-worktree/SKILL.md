...
1. **Code Review (`/js-workflows:review`)**: If NOT already on the PR branch, offer worktree for isolated review
2. **Feature Work (`/js-workflows:work`)**: Always ask if user wants parallel worktree or live branch work
...
The skill is automatically called from `/js-workflows:review` and `/js-workflows:work` commands:
