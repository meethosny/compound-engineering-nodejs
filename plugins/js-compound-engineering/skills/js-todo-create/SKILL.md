...
Load the `js-todo-triage` skill for an interactive approval workflow.
...
| Trigger | Flow |
|---------|------|
| Code review | `/js-ce:review` -> Findings -> `/js-todo-triage` -> Todos |
| Autonomous review | `/js-ce:review mode:autofix` -> Residual todos -> `/js-todo-resolve` |
| Code TODOs | `/js-todo-resolve` -> Fixes + Complex todos |
