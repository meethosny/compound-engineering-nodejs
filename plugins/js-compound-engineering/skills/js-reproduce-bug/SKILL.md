...
```
mcp__plugin_js-compound-engineering_pw__browser_navigate({ url: "http://localhost:3000" })
mcp__plugin_js-compound-engineering_pw__browser_snapshot({})
```
...
```
mcp__plugin_js-compound-engineering_pw__browser_navigate({ url: "http://localhost:3000/[affected_route]" })
mcp__plugin_js-compound-engineering_pw__browser_snapshot({})
```
...
```
mcp__plugin_js-compound-engineering_pw__browser_take_screenshot({ filename: "bug-[issue]-step-1.png" })
```
...
3. **Check for console errors:**
   ```
   mcp__plugin_js-compound-engineering_pw__browser_console_messages({ level: "error" })
   ```
...
```
mcp__plugin_js-compound-engineering_pw__browser_take_screenshot({ filename: "bug-[issue]-reproduced.png" })
```
...
- [ ] Document all research findings with specific file paths (e.g., `src/services/example-service.ts:42`)
