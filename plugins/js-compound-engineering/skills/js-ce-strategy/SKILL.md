---
name: js-ce-strategy
description: "Create or update STRATEGY.md. Use when starting a product, changing direction or roadmap, or when js-ce-ideate, js-ce-brainstorm, or js-ce-plan need upstream product grounding."
...
1. **Anchor, not plan.** Strategy is what the product is and why. Features belong in `js-ce-brainstorm`; schedules belong in the issue tracker. Do not let either creep into the doc.
...
After writing, note in one line where the file lives and that `js-ce-ideate`, `js-ce-brainstorm`, and `js-ce-plan` will pick it up as grounding on their next run.

If no downstream skill has run yet on this repo, suggest `js-ce-ideate` or `js-ce-brainstorm` skills as a next step.
...
- Does not write product requirements or implementation plans - those are `js-ce-brainstorm` and `js-ce-plan`.
