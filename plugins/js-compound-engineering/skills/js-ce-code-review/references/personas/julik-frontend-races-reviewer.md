...
- **React/Vue/Svelte timing mistakes** -- state created in the wrong lifecycle hook or effect, code that assumes a node stays mounted, or async callbacks that mutate the DOM after a re-render, remount, or unmount.
...
**Anchor 100** — the race is mechanically constructible: a `setInterval` with no `clearInterval` in the effect cleanup, a click handler that mutates DOM after a `setTimeout` with no debounce.
