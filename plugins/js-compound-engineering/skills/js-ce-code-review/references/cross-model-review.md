...
Invoke it via the skill-dir anchor — set `SKILL_DIR` to the absolute directory of **this** skill's `SKILL.md` (the one you read to run js-ce-code-review), because the Bash tool's CWD is the user's project, not the skill dir, on every host:

```bash
SKILL_DIR="<absolute path of the directory containing the js-ce-code-review SKILL.md you read>"
...
- `<run-dir>` = the Stage 4 run dir (`/tmp/compound-engineering/js-ce-code-review/<run-id>/`). The script writes `adversarial-<peer>.json` there.
