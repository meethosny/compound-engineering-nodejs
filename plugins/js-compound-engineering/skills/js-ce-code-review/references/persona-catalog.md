...
| `data-migration` | `data-migration-reviewer` | Migration files, schema files (`prisma/schema.prisma`, `prisma/migrations/`, Knex `migrations/*`), backfill scripts, data transformations — **not** model/query-only changes without migration artifacts |
...
| Persona | Agent | Select when diff touches... |
|---------|-------|---------------------------|
| `kieran-nodejs` | `kieran-nodejs-reviewer` | Node.js application code — route handlers, services, middleware, or modules — where the clarity, conventions, and maintainability of the changed code are at stake |
| `julik-frontend-races` | `julik-frontend-races-reviewer` | React/Vue/Svelte components, hooks/effects, DOM event wiring, timers, async UI flows, animations, or frontend state transitions with race potential |
...
4. **For `data-migration`**, spawn only when the diff includes migration or schema artifacts (`prisma/migrations/*`, `prisma/schema.prisma`, Knex `migrations/*`, node-pg-migrate, Alembic/Flyway/Liquibase paths, or explicit backfill/data-transform scripts). Do **not** spawn for model-only or query-only changes without those files.
