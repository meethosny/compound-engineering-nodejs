...
| **`mode:agent`** | One JSON object (see ### JSON output format below) + the same `/tmp/.../js-ce-code-review/<run-id>/` artifacts |
...
**Stack-specific conditional (per diff):** `julik-frontend-races-reviewer` (React/Vue/Svelte, DOM events, async UI) and `swift-ios-reviewer` (Swift/SwiftUI/UIKit, entitlements, Core Data, `.pbxproj`).
...
- `docs/brainstorms/*` -- legacy requirements documents created by older js-ce-brainstorm versions
- `docs/plans/*.{md,html}` -- unified plan artifacts created by js-ce-brainstorm or js-ce-plan (decision artifacts; execution progress is derived from git, not stored in plan bodies)
...
- Unified artifact: metadata includes `artifact_contract: js-ce-unified-plan/v1`.
...
if [ -z "${DIFF_A:-}" ] || ! git rev-parse --verify --quiet "${DIFF_A}^{commit}" >/dev/null 2>&1 \
  ...
  grep -qiE '\.(tsx|jsx|vue|svelte|css|scss)$|/components?/' <<< "$FILES" && echo "  frontend" || true
...
Stack-specific personas are additive when runtime behavior warrants them. A React/Vue/Svelte UI change may warrant `julik-frontend-races`; a TypeScript API diff may warrant `api-contract` and `reliability`.

**`data-migration` spawn gate.** Select `data-migration-reviewer` only when the diff includes at least one migration or schema artifact: `prisma/migrations/*`, `prisma/schema.prisma`, Knex `migrations/*`, Alembic/Flyway/Liquibase migration paths, or explicit backfill/data-transform scripts (`node scripts/backfill.js`, one-off data-migration scripts). **Do not spawn** for model-only changes, query-only refactors, serializers/controllers that reference columns without a migration or schema file in the diff, or migration tests alone.
...
mkdir -p "/tmp/compound-engineering/js-ce-code-review/$RUN_ID"
```

Pass `{run_id}` to every persona sub-agent so they can write their full analysis to `/tmp/compound-engineering/js-ce-code-review/{run_id}/{reviewer_name}.json`.
...
Each persona sub-agent writes full JSON (all schema fields) to `/tmp/compound-engineering/js-ce-code-review/{run_id}/{reviewer_name}.json` and returns compact JSON with merge-tier fields only:
  ...
  "findings": [
    {
      ...
      "pre_existing": false,
      "suggested_fix": "Add an ownership check (account.userId === req.user.id) before returning the account",
      "first_evidence": "orders.controller.ts:42 -- const account = await prisma.account.findUnique({ where: { id: req.params.accountId } })"
    }
  ],
  "residual_risks": [...],
  "testing_gaps": [...]
...
3. **Spawn validators with bounded parallelism.** One sub-agent per finding, dispatched independently using the validator template and the same bounded scheduler from Stage 4. Each validator receives:
   ...
   - `why_it_matters` when available — loaded from the per-agent artifact file at `/tmp/compound-engineering/js-ce-code-review/{run_id}/{reviewer_name}.json`; omit when the file is absent or the artifact write failed. The validator proceeds without it, using the diff and cited code directly.
   - The full diff
...
Emit **one raw JSON object** as the primary response — a single bare JSON value, **no markdown code fence**. A leading ```` ```json ```` fence makes the response start with backticks and breaks naive `JSON.parse` consumers, so never wrap it. Also write `review.json` under `/tmp/compound-engineering/js-ce-code-review/<run-id>/` with the same payload.
...
```json
{
  ...
  "testing_gaps": [],
  "coverage": {},
  "artifact_path": "/tmp/compound-engineering/js-ce-code-review/<run-id>/",
  "run_id": "<run-id>"
}
```
...
- Include the run-artifact path when one was written: `/tmp/compound-engineering/js-ce-code-review/<run-id>/`
...
Always write run artifacts under `/tmp/compound-engineering/js-ce-code-review/<run-id>/`:
