...
1. **Resolve the target directory and extension.**
   ...
   - **Otherwise (no repo, or elsewhere with no `docs/ideation/`):** write into the run's CE temp area — the `<scratch-dir>` resolved in Phase 1 (`/tmp/compound-engineering/js-ce-ideate/<run-id>/`). Do **not** write into the user's current working directory, and do **not** create a `docs/ideation/` tree for a subject unrelated to the repo. Announce the absolute path and note it is temporary (`/tmp` is cleared on reboot — move it to keep it).
...
2. **Brainstorm one idea with `js-ce-brainstorm`** — commit a chosen idea to a requirements-only unified plan under `docs/plans/`; leaves js-ce-ideate. Asks which idea first.
...
- **Markdown — Publish to Proof.** The local markdown file already exists (Phase 4) and stays canonical; Proof is a one-way published copy, not a sync target. Load the `js-ce-proof` skill to publish, passing:
...
- acting on an idea routes to `js-ce-brainstorm` (with a substance seed, not the whole file), not directly to implementation
