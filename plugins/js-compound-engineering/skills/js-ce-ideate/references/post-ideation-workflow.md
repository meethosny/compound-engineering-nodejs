...
| Brainstorm handoff | `js-ce:brainstorm` | `js-ce:brainstorm` (universal-brainstorming) |
...
Hand off the ideation content to the `js-ce-proof` skill in HITL review mode. This uploads the doc, runs an iterative review loop (user annotates in Proof, agent ingests feedback, applies agreed edits, and replies/resolves in-thread), and (in repo mode) syncs the reviewed markdown back to `docs/ideation/`.

Load the `js-ce-proof` skill in HITL-review mode with:
...
- **recommended next step:** `/js-ce:brainstorm` (shown in the proof skill's final terminal output)
...
- **§6.3 Brainstorm a selected idea.** On a successful Proof return (`proceeded` or `done_for_now`), do **not** stop at the Phase 6 menu — after applying the per-status handling below (including any stale-local pull offer), continue into §6.3's remaining bullets (mark the chosen idea as `Explored`, then load `js-ce-brainstorm`). Only the `aborted` branch returns to the Phase 6 menu, since no durable record was written.
...
3. **Brainstorm a selected idea** — load `js-ce-brainstorm` with the chosen idea as the seed. The orchestrator first writes a durable record using the mode default in Phase 5.
...
- Load the `js-ce-brainstorm` skill with the chosen idea as the seed

**Repo mode only:** do **not** skip brainstorming and go straight to `js-ce:plan` from ideation output — `js-ce:plan` wants brainstorm-grounded requirements. In elsewhere modes, ideation (or ideation + Proof iteration) is a legitimate terminal state; brainstorming is optional deeper development of one idea, not a required next rung on an implementation ladder that does not exist in these modes.
...
The `js-ce-proof` skill performs single-retry-once internally on transient failures (`STALE_BASE`, `BASE_TOKEN_REQUIRED`) before surfacing failure. The proof skill's return contract does not expose typed error classes to callers — the orchestrator cannot distinguish retryable vs terminal failures from outside.
...
- acting on an idea routes to `js-ce:brainstorm`, not directly to implementation
