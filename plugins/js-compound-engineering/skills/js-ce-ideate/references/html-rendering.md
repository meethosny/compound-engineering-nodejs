...
the agent-consumability rules below make that work. `js-ce-doc-review` is
...
`js-ce-brainstorm` writes (now under `docs/plans/`), not an implementation-ready
plan (`js-ce-plan`'s enriched output) — and only to UI-shaped requirements — a
...
Downstream agents that read HTML today (`js-ce-work`, a skill re-reading its
...
script-style parse. `js-ce-doc-review` is not a current HTML consumer (see
opening note).
