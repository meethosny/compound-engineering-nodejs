...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("source_path", type=Path, help="Path to a feedback session zip, video, audio, or meeting notes file")
    parser.add_argument(
        "--output-dir",
        type=Path,
        help="Directory for extracted artifacts. Defaults to docs/brainstorms/feedback-analysis/<source-stem> when available.",
    )
    parser.add_argument("--topic", help="Kebab-case topic for requirements-kickoff frontmatter")
    parser.add_argument(
        "--model",
        default=os.environ.get("FEEDBACK_TRANSCRIBE_MODEL", "gpt-4o-mini-transcribe"),
...
def write_requirements_kickoff(
    ...
    lines.extend(
        [
            ...
            "- `js-ce-brainstorm` can confirm requirements from linked source evidence before any planning begins.",
            "",
            "---",
            "",
            "## Scope Boundaries",
            ...
            "- Brainstorm before plan: Use `js-ce-brainstorm` to refine product behavior when the recording reveals ambiguity.",
            "",
            "---",
            "",
            ...
            "## Next Steps",
            "",
            "-> Resume `/js-ce:brainstorm` to confirm candidate findings and replace generic R-items with product-specific requirements.",
        ]
    )
    output_path.write_text("\n".join(lines) + "\n")
...
def main() -> int:
    ...
    print(f"Brainstorm handoff: /js-ce:brainstorm {display_path(kickoff_md, repo_root)}")
