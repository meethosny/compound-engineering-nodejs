...
**Headless mode** changes the interaction model, not the classification boundaries. js-ce-doc-review still applies the same judgment about which tier each finding belongs in. The only difference is how non-safe_auto findings are delivered:
...
```
Skill("js-ce-doc-review", "mode:headless docs/plans/my-plan.md")
```
...
**If no document is specified (headless mode):** Output "Review failed: headless mode requires a document path. Re-invoke with: Skill(\"js-ce-doc-review\", \"mode:headless <path>\")" without dispatching agents.
...
- `artifact_contract: js-ce-unified-plan/v1` plus `artifact_readiness: requirements-only` -> classify as `unified-requirements`. Review the Product Contract only; the absence of Planning Contract, Implementation Units, Verification Contract, or Definition of Done is expected and must not be flagged.
- `artifact_contract: js-ce-unified-plan/v1` plus `artifact_readiness: implementation-ready` -> classify as `unified-plan`. Review Product Contract and Planning Contract with different lenses, then review Implementation Units/Verification/DoD for execution completeness.
...
- Frontmatter fields like `type: feat|fix|refactor`, `origin: docs/brainstorms/...`, or `product_contract_source: js-ce-brainstorm|js-ce-plan-bootstrap|legacy-requirements`
...
- The document is a **plan with no validated upstream Product Contract signal** (no legacy `origin:` requirements doc and no `product_contract_source: js-ce-brainstorm` or `legacy-requirements`) -- premise wasn't validated upstream
...
Cross-session persistence is out of scope. A new invocation of js-ce-doc-review on the same document starts with a fresh round 1 and no carried primer, even if prior sessions deferred findings into the document's Open Questions section.
