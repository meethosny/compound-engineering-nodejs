...
- The finding's `section` is framing-level (Problem Frame, Summary, Overview, Why, Motivation, Goals — `Summary` is the new js-ce-plan / js-ce-brainstorm template heading; `Overview` retained as legacy) OR the finding explicitly questions whether a named component should exist
...
- `unified-requirements` (requirements-only unified plan) → `js-ce-plan` (enrich in place)
- `requirements` (legacy standalone requirements doc) → `js-ce-plan`
- `unified-plan` (implementation-ready unified plan) → `js-ce-work`
- `plan` (legacy implementation plan) → `js-ce-work`
...
- Do not modify caller skills (js-ce-brainstorm, js-ce-plan, or external plugin skills that invoke js-ce-doc-review)
