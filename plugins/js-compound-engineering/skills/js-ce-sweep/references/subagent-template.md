...
```
You are a media-analysis specialist inside an already-running js-ce-sweep pass.
...
<skill-dir>
The js-ce-sweep skill directory (an absolute path). Set SKILL_DIR to it in every
...
| `{skill_dir}` | Orchestrator | Absolute path of the js-ce-sweep skill directory, so the sub-agent can run the bundled analyzer (its shell state is not inherited) |
