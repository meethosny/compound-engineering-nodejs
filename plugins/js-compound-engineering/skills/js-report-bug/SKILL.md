---
name: js-report-bug
description: Report a bug in the js-compound-engineering plugin
...
Report bugs encountered while using the js-compound-engineering plugin. This command gathers structured information and creates a GitHub issue for the maintainer.
...
```bash
# Get plugin version
cat ~/.claude/plugins/installed_plugins.json 2>/dev/null | grep -A5 "js-compound-engineering" | head -10 || echo "Plugin info not found"
...
---
*Reported via `/js-report-bug` command*
```
...
```bash
gh issue create \
  --repo meethosny/compound-engineering-nodejs \
  --title "[js-compound-engineering] Bug: [Brief description]" \
  --body "[Formatted bug report from Step 3]" \
  --label "bug,js-compound-engineering"
```
...
```
Issue: https://github.com/meethosny/compound-engineering-nodejs/issues/[NUMBER]
Title: [js-compound-engineering] Bug: [description]

Thank you for helping improve the js-compound-engineering plugin!
