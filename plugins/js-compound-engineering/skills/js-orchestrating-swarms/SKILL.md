...
Task({
  ...
  subagent_type: "js-compound-engineering:review:js-security-sentinel",
...
From the `js-compound-engineering` plugin (examples):
...
// Performance review
Task({
  subagent_type: "js-compound-engineering:review:js-performance-oracle",
...
// Node.js code review
Task({
  subagent_type: "js-compound-engineering:review:js-kieran-nodejs-reviewer",
  description: "Node.js review",
  prompt: "Review this Node.js code for best practices"
})

// Architecture review
Task({
  subagent_type: "js-compound-engineering:review:js-architecture-strategist",
...
// Code simplicity
Task({
  subagent_type: "js-compound-engineering:review:js-code-simplicity-reviewer",
  description: "Simplicity check",
  prompt: "Check if this implementation can be simplified"
})
```

**All review agents from js-compound-engineering:**
- `js-agent-native-reviewer` - Ensures features work for agents too
- `js-architecture-strategist` - Architectural compliance
- `js-code-simplicity-reviewer` - YAGNI and minimalism
- `js-data-integrity-guardian` - Database and data safety
- `js-data-migration-expert` - Migration validation
- `js-deployment-verification-agent` - Pre-deploy checklists
- `js-modern-nodejs-reviewer` - Modern Node.js patterns
- `js-julik-frontend-races-reviewer` - JavaScript race conditions
- `js-kieran-python-reviewer` - Python best practices
- `js-kieran-nodejs-reviewer` - Node.js best practices
- `js-kieran-typescript-reviewer` - TypeScript best practices
- `js-pattern-recognition-specialist` - Design patterns and anti-patterns
- `js-performance-oracle` - Performance analysis
- `js-schema-drift-detector` - Schema and migration drift detection
- `js-security-sentinel` - Security vulnerabilities

### Research Agents
```javascript
// Best practices research
Task({
  subagent_type: "js-compound-engineering:research:js-best-practices-researcher",
  description: "Research auth best practices",
  prompt: "Research current best practices for JWT authentication in Node.js 2024-2026"
})

// Framework documentation
Task({
  subagent_type: "js-compound-engineering:research:js-framework-docs-researcher",
  description: "Research Fastify plugins",
  prompt: "Gather comprehensive documentation about Fastify plugin architecture"
})

// Git history analysis
Task({
  subagent_type: "js-compound-engineering:research:js-git-history-analyzer",
...
**All research agents:**
- `js-best-practices-researcher` - External best practices
- `js-framework-docs-researcher` - Framework documentation
- `js-git-history-analyzer` - Code archaeology
- `js-learnings-researcher` - Search docs/solutions/
- `js-repo-research-analyst` - Repository patterns

### Design Agents
```javascript
Task({
  subagent_type: "js-compound-engineering:design:js-figma-design-sync",
...
```javascript
Task({
  subagent_type: "js-compound-engineering:workflow:js-bug-reproduction-validator",
...
TaskCreate({
  ...
  description: "Review all files in src/services/auth/ for security vulnerabilities",
...
```json
{
  "id": "1",
  "subject": "Review authentication module",
  "description": "Review all files in src/services/auth/...",
  "status": "in_progress",
...
const research = await Task({
  ...
  prompt: "Research best practices for implementing caching in Node.js APIs. Include: cache invalidation strategies, Redis vs in-memory, cache key design."
})
...
Task({
  ...
  prompt: `
    ...
    Focus on the userController.ts endpoints.
  `
})
```
...
TaskCreate({
  ...
  description: "Extract authentication methods to AuthService",
  activeForm: "Refactoring User model..."
})

TaskCreate({
  subject: "Refactor Session controller",
  description: "Update to use new AuthService",
...
| "Agent type not found" | Invalid subagent_type | Check available agents with proper `js-` prefix |
...
// Good
prompt: `
  1. Review src/models/user.ts for N+1 queries
  2. Check all database queries have proper indexes
