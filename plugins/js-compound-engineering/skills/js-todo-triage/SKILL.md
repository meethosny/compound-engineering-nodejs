...
**Do not write code during triage.** This is purely for review and prioritization -- implementation happens in `/js-todo-resolve`.
...
**yes:** Rename file from `pending` -> `ready` in both filename and frontmatter. Fill the Recommended Action section. If creating a new todo (not updating existing), use the naming convention from the `js-todo-create` skill.
...
1. run /js-todo-resolve to resolve the todos
2. commit the todos
3. nothing, go chill
```
