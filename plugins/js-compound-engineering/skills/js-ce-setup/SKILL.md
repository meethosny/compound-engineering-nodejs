...
`js-ce-setup` is a lightweight health check and repo-local config helper. It does **not** bulk-install every optional dependency. Missing tools are reported as optional capabilities so the user can install only the workflows they use.
...
Run the bundled check script. Set `SKILL_DIR` to the absolute directory you loaded this `js-ce-setup` SKILL.md from — the Bash tool's CWD is the user's project, not the skill dir, so a bare `scripts/` path will not resolve:
...
if [ -f "$SKILL_DIR/scripts/check-health" ]; then
  ...
  echo "Bundled health script not found at $SKILL_DIR/scripts/check-health; run the inline checks from js-ce-setup instead."
fi
```
