...
- **New work (default).** No specific ref named — create a fresh branch from a base (trunk). This is what `js-ce-work` uses.
...
`js-ce-work` and `js-ce-code-review` offer this skill as an option. When the user selects "worktree" in those flows, run Step 0 first: if the work is already isolated, proceed in place; otherwise create one (native tool preferred) with a meaningful branch name derived from the work description.
