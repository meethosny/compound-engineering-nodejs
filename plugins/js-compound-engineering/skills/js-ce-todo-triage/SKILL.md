...
**Do not write code during triage.** This is purely for review and prioritization -- implementation happens in `/js-ce-todo-resolve`.

- First set the /model to Haiku
- Read all pending todos from `.context/js-compound-engineering/todos/` and legacy `todos/` directories
...
**yes:** Rename file from `pending` -> `ready` in both filename and frontmatter. Fill the Recommended Action section. If creating a new todo (not updating existing), use the naming convention from the `js-ce-todo-create` skill.
...
1. run /js-ce-todo-resolve to resolve the todos
2. commit the todos
3. nothing, go chill
```
