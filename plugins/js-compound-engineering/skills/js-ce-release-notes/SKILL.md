---
name: js-ce-release-notes
description: Summarize recent js-compound-engineering plugin releases, or answer a specific question about a past release with a version citation. Use when the user types `/js-ce-release-notes` or asks "what changed in js-compound-engineering recently?" or "what happened to `<skill-name>`?".
argument-hint: "[optional: question about a past release]"
disable-model-invocation: true
---

# js-compound-engineering Release Notes

Look up what shipped in recent releases of the js-compound-engineering plugin. Bare invocation summarizes the last 5 plugin releases. Argument invocation searches the last 40 releases and answers a specific question, citing the release version that introduced the change.

Data comes from the GitHub Releases API for `meethosny/compound-engineering-nodejs`, filtered to the `js-compound-engineering-v*` tag prefix so sibling components (`cli-v*`, `js-coding-tutor-v*`, `marketplace-v*`, `cursor-marketplace-v*`) are excluded.
...
> `python3` is required to run `/js-ce-release-notes`. Install Python 3.x and retry, or open https://github.com/meethosny/compound-engineering-nodejs/releases directly.
...
```json
{
  ...
  "fetched_at": "...",
  "releases": [
    {"tag": "js-compound-engineering-v2.67.0", "version": "2.67.0", "name": "...",
...
If `ok: true`, take the first 5 entries from `releases` (the helper has already filtered to `js-compound-engineering-v*` and sorted newest first). If fewer than 5 are available, render whatever count came back without warning.
...
```
Showing the last 5 releases. For older history, ask a specific question (e.g., `/js-ce-release-notes what happened to <skill>?`).
Browse all releases at https://github.com/meethosny/compound-engineering-nodejs/releases
```
...
```bash
gh pr view <linked_prs[0]> --repo meethosny/compound-engineering-nodejs --json title,body,url
```
...
```
I couldn't find this in the last 40 plugin releases. Browse the full history at https://github.com/meethosny/compound-engineering-nodejs/releases
```
