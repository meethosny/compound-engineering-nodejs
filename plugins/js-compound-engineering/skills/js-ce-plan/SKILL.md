...
`js-ce:brainstorm` defines **WHAT** to build. `js-ce:plan` defines **HOW** to build it. `js-ce:work` executes the plan.
...
- If the ambiguity is mainly product framing, user behavior, or scope definition, recommend `js-ce:brainstorm` first
...
- Task js-compound-engineering:research:repo-research-analyst(Scope: technology, architecture, patterns. {planning context summary})
- Task js-compound-engineering:research:learnings-researcher(planning context summary)
Collect:
...
- **Tools available + user asked**: Dispatch `js-compound-engineering:research:slack-researcher` with the planning context summary in parallel with other Phase 1.1 agents. If the origin document has a Slack context section, pass it verbatim so the researcher focuses on gaps. Include findings in consolidation.
...
- If specific frameworks and versions were detected (e.g., Node.js 7.2, Next.js 14, Go 1.22), pass those exact identifiers to framework-docs-researcher so it fetches version-specific documentation
...
- Task js-compound-engineering:research:best-practices-researcher(planning context summary)
- Task js-compound-engineering:research:framework-docs-researcher(planning context summary)
...
- Task js-compound-engineering:workflow:spec-flow-analyzer(planning context summary, research findings)
