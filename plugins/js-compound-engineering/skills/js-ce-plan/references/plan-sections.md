...
- **The implementing agent** (`js-ce-work` or a human) starts from an informed
...
`js-ce-plan` writes the canonical compound-engineering plan artifact. The same
artifact may begin as a requirements-only skeleton from `js-ce-brainstorm` and
later be enriched by `js-ce-plan`; it is still one plan file moving through
...
- **`artifact_contract: js-ce-unified-plan/v1`** — declares this contract.
- **`artifact_readiness`** — document completeness, not work progress. Valid
  ...
  - `implementation-ready` — Product Contract, Planning Contract,
    ...
    complete enough for `js-ce-work`, `/goal`, or an equivalent executor, **and no
...
- **`product_contract_source`** — where the Product Contract came from:
  `js-ce-brainstorm`, `js-ce-plan-bootstrap`, `legacy-requirements`, or another
...
`js-ce-plan` adds those implementation sections when it enriches to
...
Not every invocation of `js-ce-plan` should produce a plan document. For
genuinely atomic work, the doc is ceremony — the implementer (whether
`js-ce-work` or a human) can act directly without IDed units, KTDs, or
...
When skipping the plan doc, the work proceeds directly to `js-ce-work` or to
...
- **Implementation Units** (with stable U-IDs) — discrete work packets sized so
  ...
  Files, Approach, Test Scenarios, and Verification. `js-ce-work` and goal-mode
...
- **Verification Contract** — repo-specific commands and quality gates,
  ...
  "build time reduced 30%") and consider routing to `js-ce-optimize` — a metric
...
tracked work item. `js-ce-work` does not mutate the plan at ship time;
...
- **`origin`** — repo-relative path to an upstream brainstorm requirements
  ...
  and re-resolved when `js-ce-plan` re-deepens.
...
- **`execution`** — execution domain for downstream routing: `code`
  (the default when absent) or `knowledge-work`. `js-ce-work`'s input triage
  ...
  to the normal code path. Written by `js-ce-plan`'s approach-altitude flow
