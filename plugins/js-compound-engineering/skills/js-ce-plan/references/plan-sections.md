...
- **Implementation Units** (with stable U-IDs) — the discrete units of work,
  sized so each is independently landable. `js-ce:work` consumes these to
...
- **Sources / Research** — surface the research that orients the implementer
  ...
  Yes → surface (code locations like `src/services/reports.ts:174-176`,
...
- **`status`** — `active` on creation; `js-ce:work` flips to `completed` on
  ship. `js-ce:plan`'s Phase 0.1 resume fast path keys on `active`. In HTML,
...
- **`origin`** — repo-relative path to an upstream brainstorm requirements
  ...
  and re-resolved when `js-ce:plan` re-deepens. The Proof flow uses
