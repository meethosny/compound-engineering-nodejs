...
so the js-ce:plan handoff gates the 5.3.8 doc-review pass to `OUTPUT_FORMAT=md`
...
2. `docs/DESIGN.md`.
3. `.js-compound-engineering/DESIGN.md`.
...
- **Named font without a fetchable source.** When DESIGN.md names a
  font without a CDN URL or local `@font-face` source the agent can
  inline, treat the name as a hint about the design intent, not a
  literal directive. Emit a system-font stack in the same family
  (serif vs sans vs mono) and pick a weight that matches the intent.
  The single-file invariant still holds; do not link to an external
  stylesheet to fetch the named font.
...
- **Repo-relative code/doc paths** (`src/foo.ts`,
...
the HTML file as text linearly, not via DOM extraction. `js-ce-document-review` is
