...
This is not a software plan template (no implementation units / test scenarios) unless the deliverable itself is a software implementation plan — in which case "execute now / code" routes into the normal `js-ce-plan` flow (below) rather than composing the deliverable here.
...
**Save for later.** Persist the approach-plan to `docs/plans/` so it survives. If the deliverable is non-code, write the marker (`execution: knowledge-work`, see `references/plan-sections.md`) at persist time — so a later `js-ce-work` invocation on the saved plan routes to the carve-out, not the code path. Do not label an approach-plan as `artifact_contract: js-ce-unified-plan/v1` unless it also contains Product Contract, Planning Contract, Implementation Units, Verification Contract, and Definition of Done for a software implementation. A plan-for-a-plan is usually not suitable for `/goal`; its job is to guide a later planning or knowledge-work pass, not to execute code. Offer to deepen it. Keep the plan **agent-agnostic** (no `js-ce-work`-specific choreography in the body) so any agent can execute it later.

**Execute now -- code deliverable.** The approach-plan's job is done; continue into the normal `js-ce-plan` flow (Phase 0.1b onward) to produce the implementation plan, then hand off to `js-ce-work` for the code. `js-ce-plan` never writes the code itself.

**Execute now -- non-code deliverable.** This is the knowledge-work path with no `js-ce-work` equivalent, so it routes to `js-ce-work`'s carve-out:
...
3. Fire the `js-ce-work` skill, passing the plan path, via the platform's skill-invocation primitive (`Skill` in Claude Code). Do not merely tell the user to run it — fire it so execution happens in this session.

`js-ce-plan` itself does not execute the deliverable in any path — it produces the approach-plan and hands off. The portable plan is also runnable by any other agent without `js-ce-work`.
