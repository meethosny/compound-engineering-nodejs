...
- **Pipeline mode?** If invoked from LFG, SLFG, or any `disable-model-invocation` context: output "This is a non-software task. The LFG/SLFG pipeline requires js-ce:work, which only supports software tasks. Use `/js-ce:plan` directly for non-software planning." and stop.
...
2. **Share to Proof** — View the plan on the web and get a shareable link. Load the `js-ce-proof` skill to create and share the document. Useful for sharing with others who aren't in the terminal.
