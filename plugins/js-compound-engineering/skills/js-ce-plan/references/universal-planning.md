...
This file is loaded when js-ce-plan detects a non-software task (Phase 0.1b). It replaces the software-specific phases (0.2 through 5.1) with a domain-agnostic planning workflow.
...
- **Pipeline mode?** If invoked from LFG or any `disable-model-invocation` context: output "This is a non-software task. The LFG pipeline requires js-ce-work, which only supports software tasks. Use `/js-ce-plan` directly for non-software planning." and stop.
- **Unified artifact guard.** Universal-planning outputs are not software implementation plans. Do not label them `artifact_contract: js-ce-unified-plan/v1` and do not produce a `/goal` launch block unless the deliverable itself is a complete software implementation plan with Product Contract, Planning Contract, Implementation Units, Verification Contract, and Definition of Done.
...
**Execution here is research and analysis only — never code.** Reading code and artifacts to understand them is in-bounds research; writing or running code to change the system is not — that belongs in `js-ce-work`. This keeps the planning/execution boundary intact.
...
> Wrong: "Quick note first: /js-ce-plan builds implementation plans, so I ignored the template and just answered the question. Here's what the data says..."
...
When research is recommended, do it — don't just offer. Stale recommendations (closed restaurants, rethemed attractions, outdated prices) are worse than no recommendations. The user invoked `/js-ce-plan` because they want a good plan, not a disclaimer about training data.
...
2. **Publish to Proof — shareable link** — Publish the doc to Every's Proof editor and get a shareable link to read, comment on, or share with others. Load the `js-ce-proof` skill to create the shared document and return the URL. One-way: nothing syncs back to disk.
...
Do not offer `/js-ce-work` (software-only) or issue creation (not applicable to non-software plans).
