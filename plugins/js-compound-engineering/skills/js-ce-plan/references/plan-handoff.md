...
**Options:**
1. **Start `/js-ce:work`** - Begin implementing this plan in the current environment (recommended)
...
5. **Start `/js-ce:work` in another session** - Begin implementing in a separate agent session when the current platform supports it
...
- **`/js-ce:work`** -> Call `/js-ce:work` with the plan path
- **`/js-ce:work` in another session** -> If the current platform supports launching a separate agent session, start `/js-ce:work` with the plan path there. Otherwise, explain the limitation briefly and offer to run `/js-ce:work` in the current session instead.
...
- Ask whether to proceed to `/js-ce:work`
