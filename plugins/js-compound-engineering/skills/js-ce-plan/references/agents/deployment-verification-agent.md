...
|------|---------|-------------------|----------|----------|
| 1. Add column | `npx prisma migrate deploy` | < 1 min | N/A | Drop column |
...
```ts
// Quick sanity check
await prisma.record.count({ where: { newColumn: null, oldColumn: { not: null } } });
// Expected: 0

// Spot check random records
await prisma.$queryRaw`SELECT old_column, new_column FROM records ORDER BY RANDOM() LIMIT 10`;
