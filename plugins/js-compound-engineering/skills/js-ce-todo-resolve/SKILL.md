...
Scan `.context/js-compound-engineering/todos/*.md` and legacy `todos/*.md`. Partition by status:
...
Residual actionable work from `js-ce:review mode:autofix` after its `safe_auto` pass will already be `ready`.
...
Spawn a `js-compound-engineering:workflow:js-ce-pr-comment-resolver` agent per item. Prefer parallel; fall back to sequential respecting dependency order.
...
For large sets, use a scratch directory at `.context/js-compound-engineering/todo-resolve/<run-id>/` for per-resolver artifacts. Return only completion summaries to parent.
...
Load the `js-ce:compound` skill to document what was learned. Todo resolutions often surface patterns and architectural insights worth capturing.
...
Delete completed/resolved todo files from both paths. If a scratch directory was created at `.context/js-compound-engineering/todo-resolve/<run-id>/`, delete it (unless user asked to inspect).
...
```
Skipped pending todos (run /js-ce-todo-triage to approve):
