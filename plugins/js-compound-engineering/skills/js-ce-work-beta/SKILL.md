---
name: js-ce:work-beta
description: "[BETA] Execute work with external delegate support. Same as js-ce:work but includes experimental Codex delegation mode for token-conserving code implementation."
...
**Beta rollout note:** Invoke `js-ce:work-beta` manually when you want to trial Codex delegation. During the beta period, planning and workflow handoffs remain pointed at stable `js-ce:work` to avoid dual-path orchestration complexity.
...
If it shows an unresolved command string, read `.js-compound-engineering/config.local.yaml` from the repo root using the native file-read tool (e.g., Read in Claude Code, read_file in Codex). If the file does not exist, all settings fall through to defaults.
...
2. **Incremental Commits**
   ...
   **Commit workflow:**
   ...
   # Examples: npm test, npm test, pytest, go test, etc.
