#!/usr/bin/env bash
# Print the upstream `version` field from
# plugins/compound-engineering/.claude-plugin/plugin.json on main, or the literal
# sentinel `__CE_UPDATE_VERSION_FAILED__` if the lookup fails.
#
