...
When invoking any skill referenced below, resolve its name against the available-skills list the host platform provides and use that exact entry. Some platforms list skills under a plugin namespace (e.g., `js-compound-engineering:js-ce-plan`); others list the bare name. Invoking a short-form guess that isn't in the list will fail — always match a listed entry verbatim before calling the Skill/Task tool.
...
4. Invoke the `js-ce-review` skill with `mode:autofix plan:<plan-path-from-step-2>`.

   Pass the plan file path from step 2 so js-ce-review can verify requirements completeness.

5. Invoke the `js-ce-todo-resolve` skill.

6. Invoke the `js-ce-test-browser` skill.
