...
**Headless mode** changes the interaction model, not the classification boundaries. js-ce-document-review still applies the same judgment about which tier each finding belongs in. The only difference is how non-safe_auto findings are delivered:
...
```
Skill("js-ce-document-review", "mode:headless docs/plans/my-plan.md")
```
...
**If no document is specified (headless mode):** Output "Review failed: headless mode requires a document path. Re-invoke with: Skill(\"js-ce-document-review\", \"mode:headless <path>\")" without dispatching agents.
...
```
Reviewing with:
- js-ce-coherence-reviewer (always-on)
- js-ce-feasibility-reviewer (always-on)
- js-ce-scope-guardian-reviewer -- plan has 12 requirements across 3 priority levels
- js-ce-security-lens-reviewer -- plan adds API endpoints with auth flow
```
...
Cross-session persistence is out of scope. A new invocation of js-ce-document-review on the same document starts with a fresh round 1 and no carried primer, even if prior sessions deferred findings into the document's Open Questions section.
