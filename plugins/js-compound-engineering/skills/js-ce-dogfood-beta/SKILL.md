...
This is **diff-scoped**, not whole-app exploration. You test what *this branch* introduced or modified versus `main`. (For full-app exploratory QA, use the `js-ce-dogfood` skill instead.)
...
- A local dev server you can start (`npm run dev`, `pnpm dev`, `yarn dev`, `node server.js`, etc.).
- `agent-browser` installed. Check:
  ...
  If not installed, run the `js-ce-setup` skill to install dependencies, then resume. Do not continue without it.

## Reusing Compound-Engineering Skills

`js-ce-dogfood-beta` is an orchestrator. Prefer delegating to existing CE skills over re-deriving their behavior:
...
8. **Learnings** — reusable lessons worth carrying forward (feed substantial ones to `js-ce-compound`).
