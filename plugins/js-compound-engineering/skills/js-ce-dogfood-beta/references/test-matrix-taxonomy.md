...
|--------------|----------------|
| page/route files (`src/app/<x>/*`, `pages/<x>/*`, `routes/<x>/*`) | The pages for `<x>` (list, detail, new, edit) |
...
| API route / route handler / controller | The routes it serves and the pages that call them |
| helper / util used in views | Pages relying on it |
| client-side script / hook / event handler | Pages where that behavior is wired |
