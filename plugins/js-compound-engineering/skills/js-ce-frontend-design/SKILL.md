...
3. **agent-browser CLI** -- if nothing else is available and `agent-browser` is installed, use it. If not installed, inform the user: "`agent-browser` is not installed. Run `/js-ce-setup` to install required dependencies." Then skip to the next option.
...
For iterative refinement beyond a single pass (multiple rounds of screenshot-assess-fix), see the `js-compound-engineering:design:design-iterator` agent.
