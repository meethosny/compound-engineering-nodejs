...
2. `/js-ce:plan $ARGUMENTS`
   ...
   GATE: STOP. Verify that `/js-ce:plan` produced a plan file in `docs/plans/`. If no plan file was created, run `/js-ce:plan $ARGUMENTS` again. Do NOT proceed to step 3 until a written plan exists.
...
6. `/js-compound-engineering:js-resolve_todo_parallel`

7. `/js-compound-engineering:js-test-browser`

8. `/js-compound-engineering:js-feature-video`
