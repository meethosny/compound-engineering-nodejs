...
- `js-ce-ideate` — Phase 1 grounding, always-on for both repo and elsewhere modes (with skip-phrase opt-out).
- `js-ce-plan` — Phase 1.3 external research, dispatched for the landscape/option-discovery intent (competitor scans, prior-art, unsettled external option sets).

Other skills that need structured external grounding (for example, `js-ce-brainstorm`) can adopt this agent in follow-up work; the output contract above is stable.
