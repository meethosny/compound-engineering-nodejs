---
name: js-ce-repo-research-analyst
...
- **Data layer:** Evaluate independently from API surface -- a CLI or worker can have a database without any HTTP layer. Skip only if 0.1 found no database-related dependency (e.g., prisma, sequelize, typeorm, drizzle, knex, mongoose) **and** the root listing shows no data-related directories (`prisma/`, `migrations/`, `models/`). Otherwise, check the data layer table below.
...
| Route / controller directories (`routes/`, `src/routes/`, `src/api/`, `src/controllers/`) | HTTP routing patterns |

Data layer (skip if no database library, ORM, or migration tool in 0.1):

| File / Pattern | What it reveals |
|----------------|-----------------|
| Migration directories (`migrations/`, `prisma/migrations/`) | Database structure |
| ORM model directories (`src/models/`, `models/`) | Data model patterns |
| Schema files (`prisma/schema.prisma`, `schema.sql`) | Data model definitions |
