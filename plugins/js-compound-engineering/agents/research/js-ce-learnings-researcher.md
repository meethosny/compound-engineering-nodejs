---
name: js-ce-learnings-researcher
...
- **Architecture patterns** — structural decisions about agents, skills, services, pipelines, or system boundaries
- **Design patterns** — reusable non-architectural design approaches (content generation, interaction patterns, prompt shapes)
- **Tooling decisions** — language, library, or tool choices with durable rationale (e.g., picking a test runner, an ORM, a bundler)
...
- **Technical terms** — e.g., "N+1", "caching", "authentication", "event loop"
- **Problem indicators** — e.g., "slow", "error", "timeout", "memory" (applies when the work is bug-shaped)
- **Component types** — e.g., "service", "controller", "route", "middleware", "worker", "api"
...
When no relevant learnings are found, say so explicitly, include the search context so the caller can see what was looked for, and note that the caller's work may be worth capturing with the `js-ce-compound` skill after it lands — the absence is itself useful signal.
...
- `/js-ce:plan` — to inform planning with institutional knowledge and add depth during confidence checking
- `/js-ce:review`, `/js-ce:optimize`, `/js-ce:ideate` — to surface prior learnings relevant to the change, optimization target, or ideation topic
