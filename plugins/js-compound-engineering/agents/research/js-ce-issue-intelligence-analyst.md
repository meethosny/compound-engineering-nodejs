---
name: js-ce-issue-intelligence-analyst
...
3. Cluster by **root cause or system area**, not by symptom. Example: 25 issues mentioning `CONNECTION_REFUSED` and 5 mentioning `TIMEOUT_EXCEEDED` are different symptoms of the same systemic concern -- "service connectivity reliability." Cluster at the system level, not the error-message level.
...
- `js-ce:ideate` -- as a third parallel Phase 1 scan when issue-tracker intent is detected
