---
name: js-ce-project-standards-reviewer
...
2. For each changed file, check its ancestor directories up to the repo root for standards files. A file like `plugins/js-compound-engineering/AGENTS.md` applies to all changes under `plugins/js-compound-engineering/`.
...
- **Broken cross-references** -- agent names that are not fully qualified (e.g., `js-ce-learnings-researcher` instead of `js-compound-engineering:research:js-ce-learnings-researcher`). Skill-to-skill references using slash syntax inside a SKILL.md where the standards say to use semantic wording. References to tools by platform-specific names without naming the capability class.
