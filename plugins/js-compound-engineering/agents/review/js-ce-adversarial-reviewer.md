---
name: js-ce-adversarial-reviewer
...
- **Individual logic bugs** without cross-component impact -- js-ce-correctness-reviewer owns these
- **Known vulnerability patterns** (SQL injection, XSS, SSRF, insecure deserialization) -- js-ce-security-reviewer owns these
- **Individual missing error handling** on a single I/O boundary -- js-ce-reliability-reviewer owns these
- **Performance anti-patterns** (N+1 queries, missing indexes, unbounded allocations) -- js-ce-performance-reviewer owns these
- **Code style, naming, structure, dead code** -- js-ce-maintainability-reviewer owns these
- **Test coverage gaps** or weak assertions -- js-ce-testing-reviewer owns these
- **API contract breakage** (changed response shapes, removed fields) -- js-ce-api-contract-reviewer owns these
- **Migration safety** (missing rollback, data integrity) -- js-ce-data-migrations-reviewer owns these
