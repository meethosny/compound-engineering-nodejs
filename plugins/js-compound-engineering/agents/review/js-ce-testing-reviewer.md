...
**Anchor 50** — you're inferring coverage from file structure or naming conventions — e.g., a new `src/utils/parser.ts` with no `tests/utils/parser.test.ts`, but you can't be certain tests don't exist in an integration test file. Surfaces only as P0 escape or via mode-aware demotion to `testing_gaps`.
