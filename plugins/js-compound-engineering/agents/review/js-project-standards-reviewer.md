---
name: js-project-standards-reviewer
...
- **Broken cross-references** -- agent names that are not fully qualified (e.g., `js-learnings-researcher` instead of `js-compound-engineering:research:js-learnings-researcher`). Skill-to-skill references using slash syntax inside a SKILL.md where the standards say to use semantic wording. References to tools by platform-specific names without naming the capability class.
