...
- **Individual logic bugs** without cross-component impact -- js-correctness-reviewer owns these
- **Known vulnerability patterns** (SQL injection, XSS, SSRF, insecure deserialization) -- js-security-reviewer owns these
- **Individual missing error handling** on a single I/O boundary -- js-reliability-reviewer owns these
- **Performance anti-patterns** (N+1 queries, missing indexes, unbounded allocations) -- js-performance-reviewer owns these
- **Code style, naming, structure, dead code** -- js-maintainability-reviewer owns these
- **Test coverage gaps** or weak assertions -- js-testing-reviewer owns these
- **API contract breakage** (changed response shapes, removed fields) -- js-api-contract-reviewer owns these
- **Migration safety** (missing rollback, data integrity) -- js-data-migrations-reviewer owns these
