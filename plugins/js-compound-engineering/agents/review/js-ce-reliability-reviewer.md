---
name: js-ce-reliability-reviewer
...
**Anchor 100** — the gap is mechanical: a `fetch(url)` with no timeout/`AbortSignal`, an infinite loop with no break, a catch block that swallows the error with no log.
