...
/**
 * Collect sibling directories an agent references in its body (e.g. a co-located
 * `scripts/` folder). These are copied alongside the generated agent skill so
 * Codex can run the agent's helper scripts. Mirrors upstream #563.
 */
...
function toCanonicalWorkflowSkillName(name: string): string | null {
  ...
  return `js-ce:${name.slice("workflows:".length)}`
}

function shouldApplyCompoundWorkflowModel(plugin: ClaudePlugin): boolean {
  return plugin.manifest.name === "js-compound-engineering"
}
