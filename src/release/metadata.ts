...
export async function getCompoundEngineeringCounts(root: string): Promise<CompoundEngineeringCounts> {
  const pluginRoot = path.join(root, "plugins", "js-compound-engineering")
...
export async function syncReleaseMetadata(options: SyncOptions = {}): Promise<MetadataSyncResult> {
  ...
  const compoundClaudePath = path.join(root, "plugins", "js-compound-engineering", ".claude-plugin", "plugin.json")
  const compoundCursorPath = path.join(root, "plugins", "js-compound-engineering", ".cursor-plugin", "plugin.json")
  const codingTutorClaudePath = path.join(root, "plugins", "js-coding-tutor", ".claude-plugin", "plugin.json")
  const codingTutorCursorPath = path.join(root, "plugins", "js-coding-tutor", ".cursor-plugin", "plugin.json")
  ...
  for (const plugin of marketplaceClaude.plugins) {
    if (plugin.name === "js-compound-engineering") {
