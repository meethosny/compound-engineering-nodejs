...
This repository primarily houses the `js-compound-engineering` coding-agent plugin and the Claude Code marketplace/catalog metadata used to distribute it.

It also contains:
- the Bun/TypeScript CLI that converts Claude Code plugins into other agent platform formats
- additional plugins under `plugins/`, such as `js-coding-tutor`
...
plugins/          Plugin workspaces (js-compound-engineering, js-coding-tutor)
...
- `js-compound-engineering` under `plugins/js-compound-engineering/`
- the Claude marketplace catalog under `.claude-plugin/`
- the converter/install CLI in `src/` and `package.json`
- secondary plugins such as `plugins/js-coding-tutor/`

Do not assume a repo change is "just CLI" or "just plugin" without checking which surface owns the affected files.

## Plugin Maintenance

When changing `plugins/js-compound-engineering/` content:

- Update substantive docs like `plugins/js-compound-engineering/README.md` when the plugin behavior, inventory, or usage changes.
...
- **Include a component scope.** The scope appears verbatim in the changelog. Pick the narrowest useful label: skill/agent name (`document-review`, `learnings-researcher`), plugin or CLI area (`js-coding-tutor`, `cli`), or shared area when cross-cutting (`review`, `research`, `converters`). Never use `js-compound-engineering` -- it's the entire plugin and tells the reader nothing. Omit scope only when no single label adds clarity.
...
- `/home/user/plugins/js-compound-engineering/skills/other-skill/file.md` -- absolute path to another skill
- `~/.claude/plugins/cache/marketplace/js-compound-engineering/1.0.0/skills/other-skill/file.md` -- absolute path to an installed plugin location
...
When in doubt: if the bug affects someone running `bun install js-compound-engineering` or `bun convert`, it's an integration or product issue, not developer-experience.
